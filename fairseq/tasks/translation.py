...
class TranslationTask(FairseqTask):
    ...
    @staticmethod
    def load_pretrained_params(args, model, cuda=False):
        ...
        model.upgrade_state_dict(pre_state_dict)
        model.load_state_dict(pre_state_dict, strict=True)
        # requires_grad = True
        # if getattr(args, 'fix_params', False):
        #     requires_grad = False
        # model.load_parameter_from_dict(model, pre_state_dict, requires_grad)
        if args.fp16:
            model.half()
        else:
            model.float()
